...
from .metanet import (Block, FusedMBConv3x3, MBConv3x3, MetaNet, SABlock, to3d,
                      to4d)
from .trans_block import crossconvhrnetlayer, convlayer, crossconvhrnetlayermoe, crossconvhrnetlayermoeup,scalablelayer,selayer,gatinglayer,attentionlayer,crossconvhrnetlayerrouter, crossconvhrnetlayerrouterv2, crossconvlayer, crossconvlayerrouterv1, crossconvlayerrouterv1onlyirouter, crossconvlayerrouterv1onlyorouter, crossconvlayerdsc, crossconvlayerdtf, crossconvlayertvam
...
class Central_Model(BaseModule):
    def __init__(self,
        ...
        TRANS_FACTORY = {
            ...
            "crossconvhrnetlayerrouterv2": crossconvhrnetlayerrouterv2,
                        ...
                        for task_name in self.tasks_set:
                            if task_name != self.local_task:
                                if 'moe' in self.trans_type_now and 'hrnet' in self.trans_type_now:
                                    ...
                                    inp_feature[
                                        self.local_task][stage] = x
            # #三、下面的是交互后的main的特征图重计算
            # for idx in range(len(cur_stage)):
            #     stage = cur_stage[idx]
            #     # forward main backbone second
            #     if 'resnet' in self.name:
            #         x = self.backbone_stage[self.local_task][stage](
            #             inp_feature[self.local_task][last_stage[idx]])+self.backbone_stage[self.local_task][stage](
            #             inp_feature[self.local_task][stage])
            #     if stage == 'pre_fc':
            #         if 'resnet' in self.name:
            #             x = torch.flatten(x, 1)
