...
from .metanet import (Block, FusedMBConv3x3, MBConv3x3, MetaNet, SABlock, to3d,
                      to4d)
from .trans_block import crossconvhrnetlayer, convlayer, crossconvhrnetlayermoe, crossconvhrnetlayermoeup,scalablelayer,selayer,gatinglayer,attentionlayer,crossconvhrnetlayerrouter, crossconvlayer, crossconvlayerrouterv1, crossconvlayerrouterv2, crossconvlayerrouterv1onlyirouter, crossconvlayerrouterv1onlyorouter, crossconvlayerdsc, crossconvlayerdtf, crossconvlayertvam
...
# class AddAuxLossHook(Hook):
#     def after_train_iter(self, runner):
#         model = runner.model.module if hasattr(runner.model, 'module') else runner.model
#         # 收集模型内部的辅助 loss
#         aux_loss = 0
#         for name, module in model.named_modules():
#             if hasattr(module, 'loss_terms') and 'loss_entropy' in module.loss_terms:
#                 aux_loss += module.loss_terms['loss_entropy']
#
#         # 加到 runner.outputs['loss'] 上
#         if 'loss' in runner.outputs:
#             runner.outputs['loss'] = runner.outputs['loss'] + aux_loss
#             runner.outputs['log_vars']['loss_entropy'] = aux_loss.item()
...
class Central_Model(BaseModule):
    def __init__(self,
        ...
        self.trans_type_now = trans_type
        trans = nn.ModuleDict()
        self.model['trans'] = trans

        # 定义一个映射表
        TRANS_FACTORY = {
            "crossconvhrnetlayer": crossconvhrnetlayer,
            "crossconvhrnetlayermoe": crossconvhrnetlayermoe,
            "crossconvhrnetlayermoeup": crossconvhrnetlayermoeup,
            "scalablelayer": scalablelayer,
            "convlayer": convlayer,
            "selayer": selayer,
            "gatinglayer": gatinglayer,
            "attentionlayer":attentionlayer,
            "crossconvhrnetlayerrouter":crossconvhrnetlayerrouter,
            "crossconvlayer": crossconvlayer,
            "crossconvlayerrouterv1":crossconvlayerrouterv1,
            "crossconvlayerrouterv2": crossconvlayerrouterv2,
            "crossconvlayerrouterv1onlyirouter": crossconvlayerrouterv1onlyirouter,
            "crossconvlayerrouterv1onlyorouter": crossconvlayerrouterv1onlyorouter,
            "crossconvlayerdtf": crossconvlayerdtf,
            "crossconvlayerdsc": crossconvlayerdsc,
            "crossconvlayertvam": crossconvlayertvam
        }

        # 根据字符串获取对应类
        trans_entry = TRANS_FACTORY[trans_type]
            ...
            if 'MTB' in self.name:
                ...
                for idx in range(len(cur_stage)):
                    ...
                    if stage in self.trans_layers:
                        for task_name in self.tasks_set:
                            if task_name != self.local_task:
                                if 'moe' in self.trans_type_now and 'hrnet' in self.trans_type_now:
                                    x = self.trans[
                                        self.local_task][task_name][stage](
                                        inp_feature[task_name],
                                        inp_feature[self.local_task][stage], stage,
                                        detach=True)
                                    inp_feature[
                                        self.local_task][stage] = x
                                elif 'router' in self.trans_type_now and 'hrnet' in self.trans_type_now:
                                    x = self.trans[
                                        self.local_task][task_name][stage](
                                        inp_feature[task_name],
                                        inp_feature[self.local_task][stage],
                                    ...
                                    inp_feature[
                                        self.local_task][stage] = x
                                elif 'routerv1' in self.trans_type_now:
                                    x = self.trans[
                                        self.local_task][task_name][stage](
                                        inp_feature[task_name],
                                        inp_feature[self.local_task][stage],layer=stage,
                                        detach=True)
                                    inp_feature[
                                        self.local_task][stage] = x
                                else:
